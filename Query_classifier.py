...
Query_classifier.py
===================
Natural Language Query Parser for Soil Moisture Analysis.
"""
...
class QueryClassifier:
    ...
    def classify(self, query: str) -> dict:
        ...
        # Remove regions from query to prevent them from interrupting date patterns
        q_for_dates = q
        for r in regions:
            q_for_dates = re.sub(rf'\b{re.escape(r)}\b', '', q_for_dates)

        # Step 5: dates
        if result['operation'] == 'comparison':
            dates = self._extract_comparison_dates(q_for_dates)
        else:
            dates = self._extract_all_date_ranges(q_for_dates)
    ...
    def _detect_operation(self, q: str) -> str:
        ...
        month_pat     = r'\b(?:jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec|january|february|march|april|june|july|august|september|october|november|december)\b'
        months        = set(re.findall(month_pat, q))

        if has_comp_kw and (len(years) >= 2 or len(regions) >= 2 or len(months) >= 2
    ...
    def _extract_all_date_ranges(self, q: str) -> list:
        ...
        sep_yr      = r'(?:\s+(?:in|of|for|at|,|the|year)\s*|\s+)*'
        ...
        md_y_pat  = rf'\b({month_pat})\s+({ordinal_pat}),?{sep_yr}(\d{{4}})\b'
        d_m_y_pat = rf'\b({ordinal_pat}),?\s*({month_pat}),?{sep_yr}(\d{{4}})\b'
        mdy_dates = []
        ...
        md_to_d_y_pat = rf'\b({month_pat})\s+({ordinal_pat})\s+(?:to|and|-)\s+({ordinal_pat}),?{sep_yr}(\d{{4}})\b'
        ...
        for m in re.finditer(
            rf'\b(?:from\s+|between\s+)?({month_pat}){sep_yr}(\d{{4}})\s+(?:to|and)\s+({month_pat}){sep_yr}(\d{{4}})\b', q
        ):
        ...
        for m in re.finditer(
            rf'\b(?:from\s+|between\s+)?({month_pat})\s+(?:to|and)\s+({month_pat}){sep_yr}(\d{{4}})\b', q
        ):
        ...
        # ── 7. Month Year ─────────────────────────────────────────────
        month_year_matches = []
        for m in re.finditer(rf'\b({month_pat}){sep_yr}(\d{{4}})\b', q):
            month_n = MONTH_MAP[m.group(1)]
            year    = int(m.group(2))
